...
La pensione effettiva e' inserita dall'utente nella dashboard. Il tool confronta quella pensione lorda con un controfattuale interamente contributivo e calcola differenza per rata, valore atteso delle prestazioni con tavole di mortalita ISTAT, tempo trascorso dal pensionamento, prestazioni gia' ricevute stimate e data in cui le pensioni cumulate raggiungono il montante virtuale. Per default la pensione futura viene rivalutata con l'inflazione costante indicata dall'utente; il capitale necessario e la copertura attuariale includono quindi un fattore di perequazione attesa, calcolato sulle probabilita' di sopravvivenza. Il montante e' una base di calcolo, non un conto individuale che viene materialmente esaurito. Il risultato individuale non viene applicato alla spesa pensionistica nazionale.

La dashboard offre due modalita': `semplificata`, con pochi input e un profilo retributivo ricostruito anche tramite indici ISTAT delle retribuzioni contrattuali quando disponibili; `accurata`, con imponibili INPS effettivi modificabili anno per anno o caricabili da CSV. Una tabella solo precompilata resta una stima, mentre la modalita accurata richiede dati annuali effettivi. Data di nascita e data di pensionamento determinano eta', mesi, coefficiente storico e tempo gia' trascorso in quiescenza; l'eta' attesa usa la speranza di vita residua a 65 anni.
